...
Tidyverse function is underscore, and then we use csv2 because we use semicolon to separates the data in your     dataframe. 
    
2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
It shows that the object is a tibble wchich behaves as a dataframe. 

3. How many columns does the object have when created with these different functions?
ncol(kings) shows five columns.

4. Show the dataset so that we can see how R interprets each column
You can use for glimpse(kings) to show the dataframe in the console or view(kings) to get the data in     environment
